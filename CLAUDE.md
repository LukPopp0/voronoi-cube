...
3. **Fibonacci guarded** (`fibonacciDistributionGuarded`, current app fibonacci path) - the improvement on the restricted variant (`feature/point-distribution`). Fixed south-pole site + a deterministic, evenly spaced **guard ring** of `G` sites at angle `phiG` up from the pole; the remaining random sites are compressed into the band above an exclusion latitude (`phiExclude = phiG*(1+marginFactor)`), generalizing the old fixed `0.2*PI` band. `G` and `phiG` scale with the total point count (`computeGuardCount`, `computePhiG`; helpers exported); `G` snaps to a multiple of 4. All knobs live in `debugSettings` (debug menu) for by-eye tuning.
   ...
   - WATERTIGHTNESS: depends on the guard-ring angle. At ring angle >= ~43 deg (Fit-cutout width >= ~0.95, or manual >= 45 deg) exports are watertight. BELOW that the bottom cut leaves REAL holes (missing caps) on ~2-4 cells per export - and the default 'cutout' mapping puts typical widths there (width 0.85 -> ~38 deg). Verified with the project's own `checkTriangulated` AND directed-edge imbalance: holes appear on EVERY seed at width 0.85, including the app default (n=18, seed 1). PRE-EXISTING bottom-cut robustness limitation (old distribution breaks at low angles / exact `bottomCutoutWidth = 1.0` too). So printable today only in the >= ~0.95 band (or manual ring >= 45 deg) until the bottom-cut fix. Measure true holes as directed-edge IMBALANCE (fwd != rev) on the combined mesh; the per-cell `fwd!=1||rev!=1` check (checkTriangulated) ALSO flags balanced non-manifold edges (not holes) so it over-reports magnitude - but at these low angles both metrics agree real holes exist. (An earlier "~1.8 mm gaps" figure was that over-report; the true defect is a smaller set of genuine missing-cap holes.)
...
- Next, on separate branches:
  ...
  - Bottom-cut robustness (real blocker for the default, ROOT-CAUSED, not yet fixed): at guard-ring angles below ~43 deg the bottom cut leaves genuine holes (missing caps, directed-edge imbalance) on a few cells per export, on ALL seeds (default Fit-cutout maps width < ~0.95 to those low angles; verified n=18 seed 1 at width 0.85). Investigation (see the `it.fails` in `guardedDistribution.test.ts`): holes occur ONLY when BOTH cuts run (inner-cube-only and bottom-only are each clean), located exactly at world y = -innerCubeHalf where the inner-cube CAVITY-FLOOR plane and the frustum TOP plane are COINCIDENT. prepareForPrint cuts sequentially (inner cube first) and the frustum cut fails to correctly re-trim/re-cap that coincident plane (capTop=false leaves it open; capTop=true double-caps -> worse; order-swap and a final conform pass do NOT help - the caps are genuinely missing). Fix belongs in `printCutting.ts` handling of a region plane coincident with a prior cut's cap plane (D4 family) - delicate, watertight-critical. Exact `bottomCutoutWidth = 1.0` also fragile. Measure holes with directed-edge IMBALANCE (`holeCount` in meshInvariants.ts), not the over-reporting per-cell `fwd!=1||rev!=1` check.
