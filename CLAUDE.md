...
- `feature/print-cut-preview` DONE (branched off point-distribution): `debugSettings.previewPrintCuts` toggle in the debug menu renders the print-cut result live in the viewport (worker cuts a display-only copy via `prepareForPrint`; raw `cellData` unchanged, so export is not double-cut). See "Worker architecture" above. No new tests (cut path already covered; preview only reroutes which cell gets triangulated); 112 total green, tsc + build clean, verified in-browser.
